...
最新验证日期 20250327
验证人 wentao
上电后如果扬声器直接播放canon音乐 则表示8311及功放和扬声器工作正常
 */
...
static esp_err_t es8311_codec_init(void)
{
    ...
    ESP_LOGI(TAG, "ES8311 create success");
    ...
    ret = es8311_sample_frequency_config(es_handle, EXAMPLE_SAMPLE_RATE * EXAMPLE_MCLK_MULTIPLE, EXAMPLE_SAMPLE_RATE);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "ES8311 sample frequency config failed");
        return ret;
    }
    ESP_LOGI(TAG, "ES8311 sample frequency config success");
    ...
    ESP_LOGI(TAG, "ES8311 volume set success");

    ret = es8311_microphone_config(es_handle, false);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "ES8311 microphone config failed");
        return ret;
    }
    ESP_LOGI(TAG, "ES8311 microphone config success");
...
void app_main(void)
{
    ...
    // 初始化GPIO并设置为输出模式
    esp_rom_gpio_pad_select_gpio(03);
    gpio_set_direction(03, GPIO_MODE_OUTPUT);
    // 设置高电平
    gpio_set_level(03, 1);
